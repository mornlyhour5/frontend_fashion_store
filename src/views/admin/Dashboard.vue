...
// Replace with real dashboardApi.stats() call once your Laravel endpoint exists.
const stats = ref({
  revenue: '$0.00',
  orders: '0',
  customers: '0',
...
async function loadDashboard() {
  loading.value = true
  try {
    const res = await ordersApi.list({ per_page: 6, sort: '-created_at' })
    recentOrders.value = res.data.data || res.data || []
  } catch (e) {
    // Backend not wired yet — show empty state instead of crashing the UI.
    recentOrders.value = []
  } finally {
    loading.value = false
  }
...
<template>
  <div class="space-y-6">
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
      ...
      <StatCard label="Orders" :value="stats.orders" :change="8.1" :icon="ShoppingCart" />
      <StatCard label="Customers" :value="stats.customers" :change="4.6" :icon="Users" />
